...
def plot_learning_curve(latencies, aborts):
    ...
    ax[0].set_title('Learning curve for all %.d mice'%nAgents,size=14)
    for i in range(nAgents):
    ...
    if np.all(mask): #plot only succesful mice
        ...
        ax[1].set_title('Only mice that finished the maze in $10^5$ steps',size=14)
...
def plot_cumulative_rewards(cumulativeRewards,aborts, nTrials = 50):
    ...
    plt.title('Cumulative reward over {} trials'.format(nTrials),size=14)
...
def plot_vectorfield(agent, nTrials = 100, nActions = 4, eps0 = 0.9,
    ...
    for j in range(nSnapshots+1):
        ...
        ax[j,0].set_ylabel('after %.d trials'%(int(nTrials/nSnapshots)*j),size=14, rotation=0, labelpad=40)
        ax[j,1].quiver(locations[0,::nth],locations[1,::nth],vectors[j,1,0,::nth],vectors[j,1,1,::nth],pivot='mid',units='y',alpha=.6)
        if j == 0:
            ax[j,0].set_title('$alpha = 0$',size=14, y = 1.05)
